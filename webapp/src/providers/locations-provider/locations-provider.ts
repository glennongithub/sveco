...
export class LocationsProvider {
  ...
  async deleteLocation(location, waitForResolve: boolean = true)
  {
    // first update corresponding item in this master array
    // and take back the updated item.
    let response;
    let removedLocation = this.locations.find(this.findItemById, location.id);
    // Find the correct index for updatedLocation
    let locationIndex = this.locations.indexOf(removedLocation);

    //Delete the deleted location from local data
    ...
    try {
      ...
      if(waitForResolve) {
        ...
        // wait for com to finish
        response = await this.customApi.deleteLocation(location);
        // remove spinner
        updateLoader.dismiss();

      }
      else
        this.customApi.deleteLocation(location); //ignoring promise intentionally
    } catch(e) {
      console.log('catch in locations-provider .. deleteLocation waiting for resolve. :'+e.toString());
    }
